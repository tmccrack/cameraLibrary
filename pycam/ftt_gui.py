...
from ftt.ftt_main_window import Ui_MainWindow  # pyuic5 generated file
from ftt.ftt_servo import Servo
from ftt.ftt_mirror import Mirror
from ftt.ftt_image_logger import Logger
from ftt.ftt_temp import Temp
...
class AppWindow(Ui_MainWindow):
    ...
    def __init__(self, mainwindow, camera, mir_win, servo_win, log_win, temp_win):
        ...
        self.temp_win.spb_SetPoint.setValue(self.tempProp['set_point'])
    ...
    def connectSlots(self):
        ...
        self.actionLogging.triggered.connect(lambda:
                            self.log_win.initVals(self.camera.getLogInterval(),
                                                  self.camera.getLogState() ))
        self.actionLogging.triggered.connect(lambda:
                            self.log_win.show())
        self.log_win.buttonBox.accepted.connect(lambda:
                            self.camera.setLogInterval(self.log_win.spb_Interval.value()))
        self.log_win.buttonBox.accepted.connect(lambda:
                            self.camera.setLogState(self.log_win.rad_Logging.isChecked()))
        self.actionTemperature.triggered.connect(lambda:
                            self.temp_win.show)
        self.temp_win.buttonBox.accepted.connect(lambda:
                            self.camera.setTempProp)


    def setTempProp(self):
        # Set internal temp dict, pass to camera, camera return actual values
        self.tempProp['set_point'] = self.temp_win.spb_SetPoint.value()
    ...
    def updateTemp(self):
        ...
        self.temp_win.spb_ArrayTemp.setValue(self.tempProp['array_temp'])
        self.edt_TempStatus.setText(AppWindow.d_temp[str(self.tempProp['cooler_state'])])
        self.temp_win.edt_TempStatus.setText(AppWindow.d_temp[str(self.tempProp['cooler_state'])])
    ...
    def btnSetFrameClicked(self):
        ...
        self.imageDim['v_bin'] = 1
        self.imageDim['h_bin'] = 1
...
if __name__ == '__main__':
    ...
    log = Logger(MainWindow, 1, True)
    tem = Temp(MainWindow)
    ui = AppWindow(MainWindow, camera, mir, serv, log, tem)
    MainWindow.show()
    sys.exit(app.exec_())
